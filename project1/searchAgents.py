...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
        return state[0]==(True, True, True, True)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            updateList = list(state[0])
            count=0
            if not hitsWall:
                for corner in self.corners:
                    if (nextx, nexty) ==corner:
                        updateList[count]=True;
                    count=count+1
                newstate = (tuple(updateList),(nextx,nexty))   
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(corners)):
        if not state[0][i]:
            #distList.append((abs(x-corners[i][0])+abs(y-corners[i][1])))
    ...
    for s in state[0]:
        if s==True and other_corners!=0:
            other_corners-=minEdge
    
    return min(0,minDist)+other_corners # Default to trivial solution
